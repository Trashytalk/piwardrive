...
class PerformanceMonitor:
    ...
    async def collect_application_metrics(self) -> List[PerformanceMetric]:
        ...
        try:
            ...
            total_memory = sum(proc.info['memory_percent'] for proc in piwardrive_processes)
        ...
        with sqlite3.connect(self.database_path) as conn:
            for metric in metrics:
                ...
                """, (
                    ...
                    metric.threshold, metric.category, json.dumps(metric.tags)
                ))
            conn.commit()
    ...
    def send_alert(self, metric: PerformanceMetric):
        ...
        message = f"Performance Alert: {metric.name} = {metric.value:.2f} {metric.unit} " \
        ...
        with sqlite3.connect(self.database_path) as conn:
            ...
            """, (
                metric.name, metric.value, metric.threshold, severity, message, time.time()
            ))
            conn.commit()
    ...
    def send_email_alert(self, message: str, severity: str):
        ...
        try:
            ...
            server = smtplib.SMTP(smtp_config['email_smtp_server'], smtp_config['email_smtp_port'])
            server.starttls()
