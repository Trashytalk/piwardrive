...
try:
    ...
    from piwardrive.migrations.runner import get_applied_migrations, get_available_migrations
...
async def check_migration_status() -> Dict[str, Any]:
    """Check current migration status and identify gaps."""
    status = {
    ...
    try:
        async with _get_conn() as conn:
            ...
            try:
                ...
                status["applied_migrations"] = sorted(applied)
            ...
            try:
                ...
                status["available_migrations"] = sorted([m.version for m in available])
            except Exception as e:
                logger.warning(f"Could not get available migrations: {e}")
                status["available_migrations"] = []
            
            # Find missing migrations
            applied_set = set(status["applied_migrations"])
            ...
            status["indexes_present"] = indexes
            ...
            if status["missing_migrations"]:
                recommendations.append({
                    ...
                    "description": f"Apply {len(status['missing_migrations'])} missing migrations",
                    "migrations": status["missing_migrations"]
                })
            ...
            if missing_tables:
                recommendations.append({
                    ...
                    "description": f"Create missing tables: {', '.join(missing_tables)}",
            ...
            if missing_indexes:
                recommendations.append({
                    ...
                    "description": f"Create missing performance indexes: {', '.join(missing_indexes)}",
...
async def main():
    """Main function to check and report migration status."""
    ...
    if status["missing_migrations"]:
        print(f"\n🔴 MISSING MIGRATIONS: {status['missing_migrations']}")
    else:
