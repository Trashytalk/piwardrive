#!/usr/bin/env python3
"""
Database Schema Migration Script for PiWardrive Enhanced Schema

This script implements the comprehensive database schema outlined in database_improvements.md
"""
...
class EnhancedSchemaManager:
    """Manages enhanced database schema migrations."""
    ...
    def __init__(self):
        self.migrations: List[Migration] = [
            ...
            self._migration_12_performance_indexes,
            self._migration_13_materialized_views,
            self._migration_14_analysis_functions,
        ]
    
    async def _migration_5_wifi_detections(self, conn: aiosqlite.Connection) -> None:
        """Enhanced WiFi detections table with comprehensive fields."""
        logger.info("Creating enhanced wifi_detections table...")
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_wifi_detections_signal ON wifi_detections(signal_strength_dbm)",
            "CREATE INDEX IF NOT EXISTS idx_wifi_detections_channel ON wifi_detections(channel)",
            "CREATE INDEX IF NOT EXISTS idx_wifi_detections_encryption ON wifi_detections(encryption_type)",
            "CREATE INDEX IF NOT EXISTS idx_wifi_detections_vendor ON wifi_detections(vendor_name)",
        ]
        
        for index_sql in indexes:
            await conn.execute(index_sql)
        
        logger.info("Enhanced wifi_detections table created successfully")
    
    async def _migration_6_bluetooth_detections(self, conn: aiosqlite.Connection) -> None:
        """Bluetooth detections table."""
        logger.info("Creating bluetooth_detections table...")
    ...
    async def _migration_7_cellular_detections(self, conn: aiosqlite.Connection) -> None:
        """Cellular detections table."""
        logger.info("Creating cellular_detections table...")
    ...
    async def _migration_8_gps_tracks(self, conn: aiosqlite.Connection) -> None:
        """GPS tracking table with enhanced precision."""
        logger.info("Creating gps_tracks table...")
    ...
    async def _migration_9_network_fingerprints(self, conn: aiosqlite.Connection) -> None:
        """Network fingerprinting and classification table."""
        logger.info("Creating network_fingerprints table...")
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_fingerprints_risk ON network_fingerprints(risk_level)",
        ]
        
        for index_sql in indexes:
            await conn.execute(index_sql)
    
    async def _migration_10_suspicious_activities(self, conn: aiosqlite.Connection) -> None:
        """Security analysis and suspicious activity detection."""
        logger.info("Creating suspicious_activities table...")
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_suspicious_location ON suspicious_activities(latitude, longitude)",
        ]
        
        for index_sql in indexes:
            await conn.execute(index_sql)
    
    async def _migration_11_network_analytics(self, conn: aiosqlite.Connection) -> None:
        """Network analytics and intelligence table."""
        logger.info("Creating network_analytics table...")
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_analytics_mobility ON network_analytics(mobility_score)",
        ]
        
        for index_sql in indexes:
            await conn.execute(index_sql)
    
    async def _migration_12_performance_indexes(self, conn: aiosqlite.Connection) -> None:
        """Create composite indexes for analysis performance."""
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_bt_time_location ON bluetooth_detections(detection_timestamp, latitude, longitude)",
            "CREATE INDEX IF NOT EXISTS idx_bt_manufacturer_type ON bluetooth_detections(manufacturer_name, device_type)",
            
            # GPS tracking indexes
            "CREATE INDEX IF NOT EXISTS idx_gps_session_time ON gps_tracks(scan_session_id, timestamp)",
    ...
    async def _migration_13_materialized_views(self, conn: aiosqlite.Connection) -> None:
        """Create materialized views for analysis (SQLite doesn't support materialized views, so we'll create tables)."""
        logger.info("Creating analysis summary tables...")
        
        # Daily detection stats table (acts as materialized view)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS daily_detection_stats (
                ...
                wep_networks INTEGER,
                wpa_networks INTEGER,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                PRIMARY KEY (detection_date, scan_session_id)
            )
        """)
        
        # Network coverage grid table
        await conn.execute("""
        ...
        indexes = [
            "CREATE INDEX IF NOT EXISTS idx_daily_stats_date ON daily_detection_stats(detection_date)",
            "CREATE INDEX IF NOT EXISTS idx_coverage_grid_location ON network_coverage_grid(lat_grid, lon_grid)",
        ]
        
        for index_sql in indexes:
            await conn.execute(index_sql)
    
    async def _migration_14_analysis_functions(self, conn: aiosqlite.Connection) -> None:
        """Create database triggers and views for automated analysis."""
        logger.info("Creating analysis triggers and views...")
        
        # Trigger to update detection counts automatically
        await conn.execute("""
            ...
            BEGIN
                ...
                SET detection_count = detection_count + 1,
                    last_seen = NEW.detection_timestamp
                WHERE bssid = NEW.bssid 
                    AND id != NEW.id
                    AND ABS(julianday(NEW.detection_timestamp) - julianday(last_seen)) < 1;
            END
        """)
        ...
        await conn.execute("""
            ...
            JOIN scan_sessions ss ON sa.scan_session_id = ss.id
            WHERE sa.detected_at >= datetime('now', '-7 days')
                AND sa.false_positive = FALSE
            ORDER BY severity_score DESC, sa.detected_at DESC
        """)
        
        # View for network security overview
        await conn.execute("""
            ...
            SELECT 
                ...
                ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM wifi_detections), 2) as percentage,
                COUNT(CASE WHEN nf.risk_level = 'high' THEN 1 END) as high_risk_count,
                COUNT(CASE WHEN nf.risk_level = 'critical' THEN 1 END) as critical_risk_count
            FROM wifi_detections wd
            LEFT JOIN network_fingerprints nf ON wd.bssid = nf.bssid
            GROUP BY wd.encryption_type
    ...
    async def set_version(self, conn: aiosqlite.Connection, version: int) -> None:
        """Set schema version."""
    ...
    async def migrate(self) -> None:
        ...
        async with _get_conn() as conn:
            ...
            logger.info(f"Upgrading schema from version {current_version} to {target_version}")
            
            # Run migrations
            for i, migration in enumerate(self.migrations[start_migration:], start=start_migration):
                migration_version = 5 + i
                logger.info(f"Running migration {migration_version}...")
                
                try:
                    await migration(conn)
                    await self.set_version(conn, migration_version)
                    await conn.commit()
