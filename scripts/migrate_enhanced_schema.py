...
class EnhancedSchemaManager:
    ...
    async def _migration_6_bluetooth_detections(
    ...
    async def _migration_7_cellular_detections(
        self, conn: aiosqlite.Connection
    ) -> None:
    ...
    async def _migration_9_network_fingerprints(
        self, conn: aiosqlite.Connection
    ) -> None:
    ...
    async def _migration_10_suspicious_activities(
        self, conn: aiosqlite.Connection
    ) -> None:
    ...
    async def _migration_12_performance_indexes(
        self, conn: aiosqlite.Connection
    ) -> None:
    ...
    async def _migration_13_materialized_views(
        self, conn: aiosqlite.Connection
    ) -> None:
    ...
    async def _migration_14_analysis_functions(
        self, conn: aiosqlite.Connection
    ) -> None:
        ...
        await conn.execute(
            ...
            BEGIN
                ...
                WHERE bssid = NEW.bssid AND id != NEW.id AND ABS(julianday(NEW.detection_timestamp) - julianday(last_seen)) < 1;
            END
            """
        )
        ...
        await conn.execute(
            ...
            WHERE sa.detected_at >= datetime('now', '-7 days') AND sa.false_positive = FALSE
    ...
    async def migrate(self) -> None:
        ...
        async with _get_conn() as conn:
            ...
            if current_version >= target_version:
                ...
                return
            logger.info(
                f"Upgrading schema from version {current_version} to {target_version}"
            )
            # Run migrations
            for i, migration in enumerate(
                self.migrations[start_migration:], start=start_migration
            ):
