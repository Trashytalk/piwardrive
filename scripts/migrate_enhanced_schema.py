...
class EnhancedSchemaManager:
    ...
    async def _migration_6_bluetooth_detections(self,
    ...
    async def _migration_7_cellular_detections(self,
        conn: aiosqlite.Connection) -> None:
    ...
    async def _migration_9_network_fingerprints(self,
        conn: aiosqlite.Connection) -> None:
    ...
    async def _migration_10_suspicious_activities(self,
        conn: aiosqlite.Connection) -> None:
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_suspicious_location ON suspicious_activities(latitude,
                longitude)",
    ...
    async def _migration_12_performance_indexes(self,
        conn: aiosqlite.Connection) -> None:
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_bt_manufacturer_type ON bluetooth_detections(manufacturer_name,
                device_type)",
                

            # GPS tracking indexes
            "CREATE INDEX IF NOT EXISTS idx_gps_session_time ON gps_tracks(scan_session_id,
                timestamp)",
                

            # Analytics indexes
            "CREATE INDEX IF NOT EXISTS idx_analytics_score_date ON network_analytics(suspicious_score,
                analysis_date)",
    ...
    async def _migration_13_materialized_views(self,
        conn: aiosqlite.Connection) -> None:
        """Create materialized views for analysis (SQLite doesn't support materialized views,
            so we'll create tables)."""
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_coverage_grid_location ON network_coverage_grid(lat_grid,
                lon_grid)",
    ...
    async def _migration_14_analysis_functions(self,
        conn: aiosqlite.Connection) -> None:
    ...
    async def migrate(self) -> None:
        ...
        async with _get_conn() as conn:
            ...
            # Run migrations
            for i,
                migration in enumerate(self.migrations[start_migration:],
