...
try:
    ...
    from piwardrive.migrations.runner import run_pending_migrations, check_migration_status
...
async def run_comprehensive_migration():
    ...
    async with _get_conn() as conn:
        ...
        if status['unknown']:
            logger.warning(f"  Unknown migrations in database: {status['unknown']}")
        ...
        logger.info(f"Applying {len(status['pending'])} pending migrations...")
        applied_count = await run_pending_migrations(conn)
