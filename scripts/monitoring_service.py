...
class DatabaseMonitoringService:
        ...
        try:
            ...
            if backup_dir.exists():
                ...
                if not backup_files:
                    ...
                    if backup_age.total_seconds() > self.alert_thresholds["backup_age_hours"] * 3600:
                        alerts.append({
                            "type": "stale_backup",
                            "severity": "medium",
    ...
    async def check_maintenance_schedule(self):
        ...
        if self._should_run_maintenance("vacuum", self.maintenance_schedule["vacuum_hours"]):
            logger.info("Running scheduled vacuum...")
            try:
                result = await vacuum_database()
                logger.info(f"Vacuum completed: {result['space_reclaimed']} bytes reclaimed")
                self.last_maintenance["vacuum"] = current_time
            except Exception as e:
                logger.error(f"Vacuum failed: {e}")
        
        # Check backup schedule
        if self._should_run_maintenance("backup", self.maintenance_schedule["backup_hours"]):
            logger.info("Running scheduled backup...")
            try:
                backup_path = Path.home() / ".config" / "piwardrive" / "backups" / f"backup_{current_time.strftime('%Y%m%d_%H%M%S')}.db"
        ...
        if self._should_run_maintenance("cleanup", self.maintenance_schedule["cleanup_days"] * 24):
        ...
        if self._should_run_maintenance("validation", self.maintenance_schedule["validation_hours"]):
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            medium_severity_alerts = [a for a in recent_alerts if a.get("severity") == "medium"]
