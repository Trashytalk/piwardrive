...
class DatabaseOptimizer:
    ...
    async def analyze_query_performance(self,
        query: str,
        params: tuple = ()) -> QueryPerformanceMetric:
    ...
    async def _optimize_tables(self,
        conn: aiosqlite.Connection) -> List[Dict[str,
        Any]]:
    ...
    async def _optimize_indexes(self,
        ...
        suggested_indexes = [
            ...
            ("wifi_detections",
                "location_signal_compound",
                "latitude,
                longitude,
                signal_strength_dbm"),
                
            ("wifi_detections",
                "vendor_encryption_compound",
                "vendor_name,
                encryption_type"),
        ...
        async with _get_conn() as conn:
            ...
            for table in tables:
                ...
                usage_stats[table] = {
                    ...
                    "estimated_size_mb": row_count * len(columns) * 50 / (1024 * 1024),
                        # Rough estimate
        ...
        for table, stats in report["table_usage"].items():
            if stats["row_count"] > 10000:
                recommendations.append({
                    "type": "performance",
                    "priority": "medium",
                    "description": f"Table {table} has many rows,
...
async def main():
    ...
    print(f"\nTable Usage:")
    for table, stats in report["table_usage"].items():
        print(f"  {table}: {stats['row_count']:,
            } rows ({stats['estimated_size_mb']:.1f} MB)")
