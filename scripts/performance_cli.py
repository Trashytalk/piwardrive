...
async def analyze_database_performance(args):
    ...
    if "message" not in query_analysis:
        ...
        print(
            f"  Average time: {query_analysis.get('average_execution_time',
            0)*1000:.1f}ms"
        )
...
async def optimize_database_performance(args):
    ...
    results = await apply_performance_optimizations(
        args.database, create_indexes=not args.no_indexes
    )
    ...
    if vacuum_results:
        ...
        print(
            f"  Space saved: {vacuum_results['size_reduction_bytes']/1024/1024:.2f} MB"
        )
...
async def run_performance_benchmark(args):
    ...
    if args.database:
        ...
        print(
            f"  Total rows: {sum(stats['row_count'] for stats in table_stats.values()):,
            }"
        )
        print(
            f"  Total size: {sum(stats['size_mb'] for stats in table_stats.values()):.2f} MB"
        )
...
Examples:
  ...
  python performance_cli.py benchmark --database /path/to/database.db
    ...
    analyze_parser = subparsers.add_parser(
        "analyze-db", help="Analyze database performance"
    )
    ...
    optimize_parser = subparsers.add_parser(
        "optimize-db", help="Optimize database performance"
    )
    optimize_parser.add_argument("database", help="Path to database file")
    optimize_parser.add_argument(
        "--no-indexes", action="store_true", help="Skip index creation"
    )
    optimize_parser.add_argument(
    ...
    async_parser = subparsers.add_parser(
        "monitor-async", help="Monitor async performance"
    )
    async_parser.add_argument(
        "--duration", "-d", type=int, default=30, help="Monitoring duration in seconds"
    )
    async_parser.add_argument("--output", "-o", help="Output file for results (JSON)")

    # Real-time monitoring command
    realtime_parser = subparsers.add_parser(
        "monitor-realtime", help="Monitor real-time update performance"
    )
