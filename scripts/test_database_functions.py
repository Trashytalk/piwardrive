...
async def create_sample_data():
    ...
    session_data = {
        ...
        "location_start_lat": 40.7128,
        "location_start_lon": -74.0060,
        "location_end_lat": 40.7138,
        "location_end_lon": -74.0050,
    ...
    for i in range(5):
        wifi_detections.append({
            ...
            "detection_timestamp": (base_time + timedelta(minutes=i)).isoformat(),
            "bssid": f"aa:bb:cc:dd:ee:{i:02x}",
            "ssid": f"TestNetwork{i}",
            "channel": 6 + (i % 3) * 5,
            "frequency_mhz": 2437 + (i % 3) * 25,
            "signal_strength_dbm": -50 - (i * 5),
            "noise_floor_dbm": -95,
            "snr_db": 45 - (i * 5),
    ...
    wifi_detections.append({
        "scan_session_id": session_id,
        "detection_timestamp": (base_time + timedelta(minutes=10)).isoformat(),
        "bssid": "ff:ee:dd:cc:bb:aa",
        "ssid": "TestNetwork0",  # Same as first network
        "channel": 11,
        "frequency_mhz": 2462,
        "signal_strength_dbm": -60,
        "encryption_type": "OPEN",  # Different encryption
        "latitude": 40.7130,
        "longitude": -74.0058,
        "first_seen": (base_time + timedelta(minutes=10)).isoformat(),
    ...
    wifi_detections.append({
        "scan_session_id": session_id,
        "detection_timestamp": (base_time + timedelta(minutes=15)).isoformat(),
        ...
        "latitude": 40.7125,
        "longitude": -74.0065,
        "first_seen": (base_time + timedelta(minutes=15)).isoformat(),
    ...
    bluetooth_detections = [
        {
            "scan_session_id": session_id,
            "detection_timestamp": (base_time + timedelta(minutes=5)).isoformat(),
            ...
            "latitude": 40.7130,
            "longitude": -74.0055,
            "first_seen": (base_time + timedelta(minutes=5)).isoformat(),
    ...
    fingerprints = [
        {
            ...
            "device_model": "Test Router Model",
            "classification": "home",
            "risk_level": "low"
        }
    ]
