...
def check_license_compliance() -> Dict[str, Any]:
    ...
    if isinstance(licenses, list):
        for pkg in licenses:
            if any(prob in pkg.get("License", "").upper() for prob in problematic_licenses):
    ...
    try:
        payload = {
            ...
            'tags': {
                ...
                'critical_severity': sum(1 for v in vulnerabilities if v.get('severity') == 'critical'),
            ...
            for issue in bandit_issues:
                if issue.get('issue_severity') in ['HIGH', 'MEDIUM']:
                    vulnerabilities.append({
                        'package': 'source_code',
                        'advisory': issue.get('issue_text', 'Security issue in source code'),
...
def main():
    ...
    parser.add_argument("--security", action="store_true", help="Run security vulnerability scans")
    parser.add_argument("--enhanced-security", action="store_true", help="Run enhanced security scans with alerting")
    ...
    parser.add_argument("--full", action="store_true", help="Generate full audit report")
    parser.add_argument("--output", type=str, help="Output file for report (JSON format)")
    parser.add_argument("--no-alerts", action="store_true", help="Skip sending security alerts")
    
    args = parser.parse_args()
    
    # Default to full report if no specific checks requested
    if not any([args.outdated, args.security, args.enhanced_security, args.size, args.tree, args.licenses]):
        args.full = True
    ...
    if "enhanced_security_scan" in report:
        critical_count = sum(
            1 for vuln in report["enhanced_security_scan"]["vulnerabilities"]
