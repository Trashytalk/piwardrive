...
class PerformanceComparator:
    ...
    def compare_metrics(self, current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._has_benchmarks(current, baseline, "database"):
            db_comparison = self._compare_database_metrics(
                current["benchmarks"]["database"],
                baseline["benchmarks"]["database"]
            )
            comparison.update(self._merge_comparison(comparison, db_comparison, "database"))
        
        # Compare async metrics
        if self._has_benchmarks(current, baseline, "async"):
            async_comparison = self._compare_async_metrics(
                current["benchmarks"]["async"],
                baseline["benchmarks"]["async"]
            )
            comparison.update(self._merge_comparison(comparison, async_comparison, "async"))
        
        # Compare realtime metrics
        if self._has_benchmarks(current, baseline, "realtime"):
            rt_comparison = self._compare_realtime_metrics(
                current["benchmarks"]["realtime"],
                baseline["benchmarks"]["realtime"]
            )
            comparison.update(self._merge_comparison(comparison, rt_comparison, "realtime"))
        ...
        if self._has_benchmarks(current, baseline, "memory"):
            ...
            comparison.update(self._merge_comparison(comparison, memory_comparison, "memory"))
        
        # Update summary
        comparison["summary"]["total_metrics"] = len(comparison["regressions"]) + len(comparison["improvements"]) + len(comparison["unchanged"])
    ...
    def _has_benchmarks(self, current: Dict[str, Any], baseline: Dict[str, Any], benchmark_type: str) -> bool:
        """Check if both results have the specified benchmark type."""
        return (
            "benchmarks" in current and 
            "benchmarks" in baseline and
            benchmark_type in current["benchmarks"] and
            benchmark_type in baseline["benchmarks"] and
            not isinstance(current["benchmarks"][benchmark_type], dict) or
            "error" not in current["benchmarks"][benchmark_type]
        )
    
    def _merge_comparison(self, main_comparison: Dict[str, Any], sub_comparison: Dict[str, Any], category: str) -> Dict[str, Any]:
    ...
    def _compare_database_metrics(self, current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "insert_performance" in current and "insert_performance" in baseline:
            insert_comparison = self._compare_performance_metrics(
                current["insert_performance"],
                baseline["insert_performance"],
                "Insert"
            )
        ...
        if "update_performance" in current and "update_performance" in baseline:
            update_comparison = self._compare_performance_metrics(
                current["update_performance"],
                baseline["update_performance"],
                "Update"
            )
    ...
    def _compare_async_metrics(self, current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "task_performance" in current and "task_performance" in baseline:
            task_comparison = self._compare_performance_metrics(
                current["task_performance"],
                baseline["task_performance"],
                "Async Task"
            )
    ...
    def _compare_realtime_metrics(self, current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "connection_performance" in current and "connection_performance" in baseline:
            ...
            comparison = self._merge_metric_comparison(comparison, connection_comparison)
        
        return comparison
    
    def _compare_memory_metrics(self, current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "memory_growth_mb" in current and "memory_growth_mb" in baseline:
            ...
            change_ratio = (current_growth - baseline_growth) / baseline_growth if baseline_growth != 0 else 0
        ...
        if "peak_memory_mb" in current and "peak_memory_mb" in baseline:
            ...
            change_ratio = (current_peak - baseline_peak) / baseline_peak if baseline_peak != 0 else 0
    ...
    def _compare_performance_metrics(self, current: Dict[str, Any], baseline: Dict[str, Any], metric_name: str) -> Dict[str, Any]:
        ...
        if "mean" in current and "mean" in baseline:
            ...
            change_ratio = (current_mean - baseline_mean) / baseline_mean if baseline_mean != 0 else 0
        ...
        if "p95" in current and "p95" in baseline:
            ...
            change_ratio = (current_p95 - baseline_p95) / baseline_p95 if baseline_p95 != 0 else 0
        ...
        if "p99" in current and "p99" in baseline:
            ...
            change_ratio = (current_p99 - baseline_p99) / baseline_p99 if baseline_p99 != 0 else 0
    ...
    def _compare_batch_performance(self, current: List[Dict[str, Any]], baseline: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for batch_size in common_sizes:
            ...
            change_ratio = (current_time - baseline_time) / baseline_time if baseline_time != 0 else 0
    ...
    def _merge_metric_comparison(self, main: Dict[str, Any], sub: Dict[str, Any]) -> Dict[str, Any]:
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Compare performance metrics with baseline')
