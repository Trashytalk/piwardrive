...
class IntegrationTestFixture:
    ...
    def setup(self):
        ...
        test_config = {
            ...
            "gps": {
                "enabled": False,  # Disable GPS for testing
                "mock_data": True
            },
            "network": {
                "scan_interval": 10,
...
class TestServiceIntegration:
    ...
    def test_service_startup_and_shutdown(self, integration_fixture):
        ...
        config = ConfigManager(integration_fixture.config_path)
    ...
    def test_config_validation_and_loading(self, integration_fixture):
        ...
        invalid_config_path = os.path.join(integration_fixture.test_dir, "invalid_config.json")
    ...
    def test_data_aggregation_integration(self, integration_fixture):
        ...
        with integration_fixture.managed_service(AggregationService, config) as aggregator:
...
class TestErrorHandlingIntegration:
    ...
    def test_database_connection_failure_handling(self, integration_fixture):
        ...
        invalid_config_path = os.path.join(integration_fixture.test_dir, "invalid_db_config.json")
        with open(invalid_config_path, 'w') as f:
...
def verify_data_integrity(persistence, expected_count):
    ...
    for network in networks:
        assert all(key in network for key in ['ssid', 'bssid', 'signal_strength', 'frequency', 'timestamp'])
