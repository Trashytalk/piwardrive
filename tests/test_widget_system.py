...
from piwardrive.widget_manager import WidgetManager, WidgetRegistry
...
class TestWidgetRegistry:
    ...
    def test_widget_discovery(self):
        ...
        with patch("piwardrive.widget_manager.discover_widgets") as mock_discover:
            mock_discover.return_value = {"DiscoveredWidget": Mock(spec=BaseWidget)}
                ...
                if isinstance(data, dict):
                    ...
                    for key, value in data.items():
                        if isinstance(value, str):
                            # Remove script tags
                            sanitized[key] = value.replace("<script>", "").replace(
                                "</script>", ""
                            )
                        else:
                            sanitized[key] = value
