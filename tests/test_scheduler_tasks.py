...
class TestBackgroundTaskQueue:
    ...
    async def test_task_queue_priority(self):
        ...
        await queue.add_task(
            priority_task, TaskPriority.LOW, "low1", priority=TaskPriority.LOW
        )
        await queue.add_task(
            priority_task, TaskPriority.HIGH, "high1", priority=TaskPriority.HIGH
        )
        await queue.add_task(
            priority_task, TaskPriority.NORMAL, "normal1", priority=TaskPriority.NORMAL
        )
        await queue.add_task(
            priority_task, TaskPriority.HIGH, "high2", priority=TaskPriority.HIGH
        )
        ...
        high_priority_indices = [
            i
            for i, (priority, _) in enumerate(execution_order)
...
class TestSchedulerIntegration:
    ...
    def test_health_monitor_integration(self):
        ...
        scheduler.add_task("health_monitor", health_monitor.collect_metrics, interval=5)
    ...
    async def test_analytics_job_integration(self):
        ...
        with patch(
            "piwardrive.jobs.analytics_jobs.analyze_network_patterns"
        ) as mock_analyze:
            with patch(
                "piwardrive.jobs.analytics_jobs.detect_anomalies"
            ) as mock_detect:
    ...
    async def test_maintenance_job_integration(self):
        ...
        with patch("piwardrive.jobs.maintenance_jobs.cleanup_old_data") as mock_cleanup:
            with patch(
                "piwardrive.jobs.maintenance_jobs.optimize_database"
            ) as mock_optimize:
