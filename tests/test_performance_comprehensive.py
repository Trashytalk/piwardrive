...
class PerformanceTestFixture:
    ...
    def stop_monitoring(self, operation_name: str, operations_count: int = 1) -> PerformanceMetrics:
...
class TestDatabasePerformance:
    ...
    def test_bulk_insert_performance(self, perf_fixture):
        ...
        config = ConfigManager(perf_fixture.config_path)
        ...
        for batch_size in batch_sizes:
            with perf_fixture.performance_context(f"bulk_insert_{batch_size}", batch_size):
                # Generate test data
                test_data = []
    ...
    def test_query_performance(self, perf_fixture):
        ...
        query_tests = [
            ...
            ("get_networks_by_ssid", lambda: persistence.get_networks_by_ssid("QueryTestNetwork1000")),
            ("get_recent_networks", lambda: persistence.get_recent_networks(1000)),
            ("get_networks_by_signal", lambda: persistence.get_networks_by_signal_range(-50, -40))
        ]
    ...
    def test_api_response_performance(self, perf_fixture):
        ...
        try:
            ...
            for endpoint in endpoints:
                with perf_fixture.performance_context(f"api_response_{endpoint.replace('/', '_')}", 1):
                ...
                with perf_fixture.performance_context(f"concurrent_load_{concurrent_users}_{requests_per_user}", 
                    ...
                    with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
                        ...
                        for user_id in range(concurrent_users):
                            future = executor.submit(simulate_user_session, user_id, requests_per_user)
                            futures.append(future)
...
class TestAnalysisPerformance:
    ...
    def test_large_dataset_analysis(self, perf_fixture):
        ...
        analysis_operations = [
            ...
            ("frequency_analysis", lambda: analysis_engine.analyze_frequency_distribution()),
