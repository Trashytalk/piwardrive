...
class PerformanceTestFixture:
    ...
    def setup(self):
        ...
        perf_config = {
            ...
            "webui": {"port": 0, "host": "127.0.0.1", "workers": 4},
            "network": {"scan_interval": 1, "batch_size": 100, "mock_data": True},
            "performance": {
    ...
    def stop_monitoring(
        self, operation_name: str, operations_count: int = 1
    ) -> PerformanceMetrics:
...
class TestDatabasePerformance:
    ...
    def test_bulk_insert_performance(self, perf_fixture):
        ...
        ConfigManager(perf_fixture.config_path)
        ...
        for batch_size in batch_sizes:
            with perf_fixture.performance_context(
                f"bulk_insert_{batch_size}", batch_size
            ):
                # Generate test data
                test_data = []
    ...
    def test_concurrent_database_access(self, perf_fixture):
        ...
        for concurrency in concurrency_levels:
            with perf_fixture.performance_context(
                f"concurrent_access_{concurrency}", concurrency * operations_per_worker
            ):
                ...
                with ThreadPoolExecutor(max_workers=concurrency) as executor:
                    ...
                    for worker_id in range(concurrency):
                        future = executor.submit(
                            worker_function, worker_id, operations_per_worker
                        )
    ...
    def test_api_response_performance(self, perf_fixture):
        ...
        try:
            ...
            for endpoint in endpoints:
                with perf_fixture.performance_context(
                    f"api_response_{endpoint.replace('/','_')}", 1
                ):
    ...
    def test_concurrent_user_load(self, perf_fixture):
        ...
        try:
            ...
            for concurrent_users, requests_per_user in load_levels:
                ...
                with perf_fixture.performance_context(
                    f"concurrent_load_{concurrent_users}_{requests_per_user}",
                    total_requests,
                ):
                    ...
                    with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
                        ...
                        for user_id in range(concurrent_users):
                            future = executor.submit(
                                simulate_user_session, user_id, requests_per_user
                            )
                            futures.append(future)
...
class TestAsyncPerformance:
        ...
        async def async_data_processor(data_batch):
            ...
            for data in data_batch:
                ...
                processed_data.append(
                    {**data, "processed": True, "processed_at": time.time()}
                )
    ...
    async def test_async_concurrent_operations(self, perf_fixture):
        ...
        for concurrency in concurrency_levels:
            with perf_fixture.performance_context(
                f"async_concurrent_{concurrency}", concurrency
            ):
...
class TestStressTest:
    ...
    def test_maximum_concurrent_connections(self, perf_fixture):
        ...
        try:
            ...
            for connection_count in connection_counts:
                ...
                with perf_fixture.performance_context(
                    f"max_connections_{connection_count}", connection_count
                ):
                    ...
                    with ThreadPoolExecutor(max_workers=connection_count) as executor:
                        ...
                        successful_connections = sum(
                            1 for future in futures if future.result()
                        )
    ...
    def test_memory_leak_detection(self, perf_fixture):
        ...
        if len(memory_samples) > 1:
            memory_growth = (memory_samples[-1] - memory_samples[0]) / len(
                memory_samples
            )
    ...
    report = {
        "summary": {
            ...
            "avg_throughput": statistics.mean(
                [m.throughput for m in metrics_list if m.throughput > 0]
            ),
            "avg_latency": statistics.mean(
                [m.latency for m in metrics_list if m.latency > 0]
            ),
            "overall_success_rate": statistics.mean(
                [m.success_rate for m in metrics_list]
            ),
        },
        "detailed_results": [],
    }
