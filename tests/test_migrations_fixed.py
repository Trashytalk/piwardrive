...
class TestPerformanceIndexesMigration:
    ...
    async def test_performance_indexes_creation(self):
        ...
        async with aiosqlite.connect(":memory:") as conn:
            ...
            await conn.execute(
                "CREATE INDEX idx_wifi_detected_at ON wifi_detections(detected_at)"
            )
            await conn.execute(
                "CREATE INDEX idx_bt_address ON bluetooth_detections(device_address)"
            )
            await conn.execute(
                "CREATE INDEX idx_bt_detected_at ON bluetooth_detections(detected_at)"
            )
    ...
    async def test_complete_migration_workflow(self):
        ...
        try:
            async with aiosqlite.connect(db_path) as conn:
                ...
                await conn.execute(
                    "INSERT INTO migration_versions (version) VALUES (3)"
                )
