...
class TestPiWardriveAppCore:
    ...
    def test_app_initialization_basic(self, mock_import, mock_health_monitor, mock_poll_scheduler,
                                    mock_run_async_task, 
                                    mock_load_app_state, mock_load_config, 
                                    mock_install, mock_init_logging):
    ...
    @patch('piwardrive.main.PollScheduler')
    @patch('piwardrive.main.diagnostics.HealthMonitor')
    def test_app_admin_password_setting(self, mock_health_monitor, mock_poll_scheduler, 
                                       mock_hash_password, mock_tile_maintenance,
                                       mock_run_async_task, mock_load_app_state, 
                                       mock_load_config, mock_install, mock_init_logging):
        ...
        with patch.dict(os.environ, {'PW_ADMIN_PASSWORD': 'test_password'}, clear=False):
            app = PiWardriveApp()
    ...
    @patch('piwardrive.main.PollScheduler')
    @patch('piwardrive.main.diagnostics.HealthMonitor')
    def test_app_custom_container(self, mock_health_monitor, mock_poll_scheduler,
                                 mock_tile_maintenance, mock_run_async_task,
                                 mock_load_app_state, mock_load_config, 
                                 mock_install, mock_init_logging):
                        ...
                        with patch('piwardrive.main.utils.run_async_task'):
                            with patch('piwardrive.main.tile_maintenance', create=True):
                                with patch('piwardrive.main.PollScheduler'):
                                    with patch('piwardrive.main.diagnostics.HealthMonitor'):
                                        ...
                                        with patch.dict(os.environ, {'PW_ADMIN_PASSWORD': ''}, clear=False):
                                            app = PiWardriveApp(service_cmd_runner=mock_service_cmd_runner)
                                            
                                        # Verify custom service command runner is used
                                        assert app._run_service_cmd is mock_service_cmd_runner
...
class TestMainModuleIntegration:
    ...
    @patch('piwardrive.main.PollScheduler')
    @patch('piwardrive.main.diagnostics.HealthMonitor')
    def test_config_and_state_loading(self, mock_health_monitor, mock_poll_scheduler,
                                     mock_load_app_state, mock_load_config):
    ...
    def test_mqtt_client_initialization(self, mock_load_app_state, mock_load_config):
        ...
        with patch('piwardrive.main.init_logging'):
            with patch('piwardrive.main.exception_handler.install'):
                with patch('piwardrive.main.utils.run_async_task'):
                    with patch('piwardrive.main.tile_maintenance', create=True):
                        with patch('piwardrive.mqtt.MQTTClient', return_value=mock_mqtt_client):
                            with patch.dict(os.environ, {'PW_ADMIN_PASSWORD': ''}, clear=False):
                                from piwardrive.main import PiWardriveApp
                                app = PiWardriveApp()
    ...
    def test_scan_report_error_handling(self, mock_load_app_state, mock_load_config):
        ...
        with patch('piwardrive.main.init_logging'):
            with patch('piwardrive.main.exception_handler.install'):
                with patch('piwardrive.main.utils.run_async_task'):
                    with patch('piwardrive.main.tile_maintenance', create=True):
                        with patch('piwardrive.scan_report', side_effect=ImportError("Module not found")):
                            with patch('logging.exception') as mock_log_exception:
                                with patch.dict(os.environ, {'PW_ADMIN_PASSWORD': ''}, clear=False):
                                    ...
                                    mock_log_exception.assert_called_once_with("Failed to schedule scan report")
            ...
            with patch('piwardrive.main.load_app_state', return_value=mock_app_state):
                with patch('piwardrive.main.init_logging'):
                    with patch('piwardrive.main.exception_handler.install'):
                        with patch('piwardrive.main.utils.run_async_task'):
                            with patch('piwardrive.main.tile_maintenance', create=True):
                                with patch('piwardrive.main.hash_password') as mock_hash_password:
                                    # Set environment variable but config already has hash
                                    with patch.dict(os.environ, {'PW_ADMIN_PASSWORD': 'new_password'}, clear=False):
                                        app = PiWardriveApp()
                                        
                                        # Verify existing hash is preserved
                                        assert mock_config.admin_password_hash == "existing_hash"
