...
class TestPiWardriveAppInterface:
    ...
    def test_app_basic_attributes_assignment(
        self, mock_load_app_state, mock_load_config
    ):
        ...
        with patch("piwardrive.main.init_logging"):
            with patch("piwardrive.main.exception_handler.install"):
                with patch.object(
                    PiWardriveApp,
                    "__init__",
                    lambda self, container=None, service_cmd_runner=None: None,
                ):
                    app = PiWardriveApp(
                        container=custom_container,
                        service_cmd_runner=custom_service_runner,
                    )
