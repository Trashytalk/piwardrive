...
class TestConfigurationLoading:
    ...
    def test_load_config_with_partial_file(self, tmp_path):
        ...
        with patch("piwardrive.core.config.CONFIG_DIR", str(tmp_path)):
            ...
            config_path = tmp_path / "config.json"
            partial_config = {"map_poll_aps": 90, "debug_mode": True}
...
class TestConfigurationSaving:
    ...
    def test_save_and_load_roundtrip(self, tmp_path):
        ...
        with patch("piwardrive.core.config.CONFIG_DIR", str(tmp_path)):
            ...
            assert loaded_config.log_paths == [
                "/var/log/test1.log",
                "/var/log/test2.log",
            ]
...
class TestConfigurationValidation:
    ...
    def test_validate_config_invalid_limits(self):
        ...
        with pytest.raises(
            ConfigError, match="tile_cache_limit_mb must be non-negative"
        ):
            validate_config_data(config)
...
class TestConfigurationProfiles:
    ...
    def test_create_and_load_profile(self, tmp_path):
        ...
        with patch("piwardrive.core.config.CONFIG_DIR", str(tmp_path)):
            with patch(
                "piwardrive.core.config.PROFILES_DIR", str(tmp_path / "profiles")
            ):
                # Create a profile
                config = Config()
    ...
    def test_switch_profile(self, tmp_path):
        ...
        with patch("piwardrive.core.config.CONFIG_DIR", str(tmp_path)):
            with patch(
                ...
                with patch(
                    "piwardrive.core.config.ACTIVE_PROFILE_FILE",
                    str(tmp_path / "active_profile"),
                ):
                    # Create profiles
                    config1 = Config()
...
class TestEnvironmentOverrides:
    ...
    def test_environment_override_precedence(self, tmp_path):
        ...
        with patch("piwardrive.core.config.CONFIG_DIR", str(tmp_path)):
            ...
            with patch.dict(
                os.environ, {"PW_MAP_POLL_APS": "90", "PW_DEBUG_MODE": "true"}
            ):
                config = load_config()
...
class TestConfigurationExportImport:
    ...
    def test_export_import_roundtrip(self, tmp_path):
        ...
        with patch("piwardrive.core.config.CONFIG_DIR", str(tmp_path)):
            ...
            assert imported_config.log_paths == [
                "/var/log/app.log",
                "/var/log/error.log",
            ]
            assert imported_config.dashboard_layout == [
                {"widget": "test", "size": [100, 200]}
            ]
