            ...
            with patch("piwardrive.main.init_logging"):
                with patch("piwardrive.main.exception_handler"):
                    with patch("piwardrive.main.utils.run_async_task"):
                        with patch("piwardrive.main.tile_maintenance"):
                            with patch.dict(
                                os.environ, {"PW_ADMIN_PASSWORD": "test_password"}
                            ):
                                ...
                                assert (
                                    app.config_data.admin_password_hash
                                    != "test_password"
                                )
                                assert (
                                    len(app.config_data.admin_password_hash) > 20
                                )  # Hashed
            ...
            with patch("piwardrive.main.init_logging"):
                with patch("piwardrive.main.exception_handler"):
                    with patch("piwardrive.main.utils.run_async_task"):
                        with patch("piwardrive.main.tile_maintenance"):
                            with patch(
                                "piwardrive.diagnostics.HealthMonitor"
                            ) as mock_health:
                    ...
                    with patch("piwardrive.main.exception_handler"):
                        with patch("piwardrive.main.utils.run_async_task"):
                            with patch("piwardrive.main.tile_maintenance"):
                                ...
                                assert (
                                    app.config_data.remote_sync_url
            ...
            with patch("piwardrive.main.init_logging"):
                with patch("piwardrive.main.exception_handler"):
                    with patch("piwardrive.main.utils.run_async_task") as mock_async:
                        with patch("piwardrive.main.tile_maintenance"):
                            with patch(
                                "piwardrive.main.analytics_jobs"
                            ) as mock_analytics:
                                with patch(
                                    "piwardrive.main.maintenance_jobs"
                                ) as mock_maintenance:
                    ...
                    with patch("piwardrive.main.exception_handler"):
                        with patch("piwardrive.main.utils.run_async_task"):
                            with patch("piwardrive.main.tile_maintenance"):
                                ...
                                assert app.config_data.notification_webhooks == [
                    ...
                    with patch("piwardrive.main.exception_handler"):
                        with patch("piwardrive.main.utils.run_async_task"):
                            with patch("piwardrive.main.tile_maintenance"):
                                ...
                                with pytest.raises(
                                    Exception, match="Config loading failed"
                                ):
                                    PiWardriveApp()
                    ...
                    with patch("piwardrive.main.exception_handler"):
                        with patch("piwardrive.main.utils.run_async_task"):
                            with patch("piwardrive.main.tile_maintenance"):
                                ...
                                # Should raise the exception
                                with pytest.raises(
                                    Exception, match="App state loading failed"
                                ):
                                    PiWardriveApp()
                    ...
                    with patch("piwardrive.main.exception_handler"):
                        with patch("piwardrive.main.utils.run_async_task"):
                            with patch("piwardrive.main.tile_maintenance"):
                                with patch("piwardrive.mqtt.MQTTClient") as mock_mqtt:
                                    mock_mqtt_instance = Mock()
                                    mock_mqtt_instance.connect.side_effect = Exception(
                                    ...
                                    with pytest.raises(
                                        Exception, match="MQTT connection failed"
                                    ):
                                        PiWardriveApp()
                    ...
                    with patch("piwardrive.main.utils.run_async_task"):
                        with patch("piwardrive.main.tile_maintenance"):
                            with patch(
                                ...
                                mock_scan_report.side_effect = Exception(
                                    "Scan report failed"
                                )
                        ...
                        with patch("piwardrive.main.utils.run_async_task"):
                            with patch("piwardrive.main.tile_maintenance"):
                                with patch("piwardrive.mqtt.MQTTClient") as mock_mqtt:
                                    ...
                                    with patch.dict(
                                        os.environ, {"PW_UPDATE_INTERVAL": "12"}
                                    ):
                                        app = PiWardriveApp()
                                        ...
                                        assert (
                                            app.config_data.remote_sync_interval == 60
                                        )
                                        assert app.mqtt_client is not None
                                        assert (
                                            len(app.config_data.notification_webhooks)
                                            == 1
                                        )
