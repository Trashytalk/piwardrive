...
class TestGetVersion:
    ...
    def test_get_version_success(self):
        ...
        with patch('piwardrive.logging.structured_logger.version', return_value="1.2.3"):
            assert _get_version() == "1.2.3"
    
    def test_get_version_not_found(self):
        """Test version retrieval when package not found."""
        from importlib.metadata import PackageNotFoundError
        with patch('piwardrive.logging.structured_logger.version', side_effect=PackageNotFoundError):
...
class TestStructuredFormatter:
    ...
    def test_serialize_fallback_to_json(self):
        ...
        test_dict = {"level": "INFO", "message": "test"}
        with patch('piwardrive.logging.structured_logger.dumps', side_effect=Exception("dumps failed")):
            result = self.formatter._serialize(test_dict)
            parsed = json.loads(result)
            assert parsed["level"] == "INFO"
            assert parsed["message"] == "test"
...
class TestAdvancedPiWardriveLogger:
    ...
    def test_formatter_serialization_complete_failure(self):
        ...
        with patch('piwardrive.logging.structured_logger.dumps', side_effect=Exception("fastjson failed")):
...
class TestLoggerErrorConditions:
    ...
    def test_file_handler_creation_with_permission_error(self):
        ...
        config = {"streams": False, "file": "/invalid/path/that/should/not/exist/test.log"}
