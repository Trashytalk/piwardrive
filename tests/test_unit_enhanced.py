...
class RealDatabaseTestMixin:
    ...
    def insert_test_data(self):
        ...
        # Insert test devices
        test_devices = [
            ('aa:bb:cc:dd:ee:ff', 'Apple Inc.', 'smartphone', int(time.time()), int(time.time())),
            ('bb:cc:dd:ee:ff:00', 'Samsung', 'smartphone', int(time.time()), int(time.time())),
        ]
        
        cursor.executemany('''
            INSERT INTO devices (mac_address, manufacturer, device_type, first_seen, last_seen)
            VALUES (?, ?, ?, ?, ?)
        ''', test_devices)
...
class TestPersistenceManagerReal(RealDatabaseTestMixin):
    ...
    def test_bulk_operations_real(self):
        ...
        # Verify bulk update
        updated_networks = persistence.get_networks_by_bssid_list([data['bssid'] for data in update_data])
    ...
    def test_temporal_analysis_real_data(self):
        ...
        for i in range(60):  # 60 data points over 1 hour
            ...
            cursor.execute('''
                INSERT INTO networks (ssid, bssid, signal_strength, frequency, timestamp)
                VALUES (?, ?, ?, ?, ?)
            ''', (f'TimeSeries{i}', f'00:11:22:33:{i//10:02d}:{i%10:02d}', -45 - (i % 20), 2412, timestamp))
...
class TestNetworkScannerReal:
    ...
    def test_scan_filtering_real(self):
        ...
        raw_results = [
            {'ssid': 'TestNetwork1', 'bssid': '00:11:22:33:44:55', 'signal_strength': -30, 'frequency': 2412, 'timestamp': int(time.time())},
            {'ssid': 'TestNetwork2', 'bssid': '00:11:22:33:44:66', 'signal_strength': -80, 'frequency': 2437, 'timestamp': int(time.time())},
            {'ssid': 'TestNetwork3', 'bssid': '00:11:22:33:44:77', 'signal_strength': -45, 'frequency': 2462, 'timestamp': int(time.time())},
        ]
