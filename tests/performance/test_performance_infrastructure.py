...
class PerformanceTestRunner:
    ...
    def run_api_load_test(self, config: LoadTestConfig) -> PerformanceTestResult:
        ...
        result = PerformanceTestResult(
            ...
            latency_p99=statistics.quantiles(response_times, n=100)[98] if len(response_times) > 1 else response_times[0],
    ...
    async def run_async_load_test(self, config: LoadTestConfig) -> PerformanceTestResult:
    ...
    def run_stress_test(self) -> PerformanceTestResult:
        ...
        result = PerformanceTestResult(
            ...
            latency_p95=statistics.quantiles(response_times, n=20)[18] if len(response_times) > 1 else (response_times[0] if response_times else 0),
            latency_p99=statistics.quantiles(response_times, n=100)[98] if len(response_times) > 1 else (response_times[0] if response_times else 0),
    ...
    def compare_with_baseline(self, baseline_file: str = "performance_baseline.json") -> Dict[str, Any]:
...
class TestPerformance:
    ...
    def test_api_response_time(self, perf_runner):
        ...
        assert result.latency_avg < 2.0, f"Average latency too high: {result.latency_avg}s"
        assert result.latency_p95 < 3.0, f"95th percentile latency too high: {result.latency_p95}s"
    ...
    def test_concurrent_user_handling(self, perf_runner):
        ...
        assert result.success_rate > 0.90, f"Success rate too low under load: {result.success_rate:.2%}"
    ...
    def test_database_performance(self, perf_runner):
        ...
        assert result.latency_avg < 1.0, f"Database operations too slow: {result.latency_avg}s"
        assert result.success_rate > 0.95, f"Database success rate too low: {result.success_rate:.2%}"
    ...
    def test_memory_usage(self, perf_runner):
        ...
        assert result.memory_peak < 500, f"Memory usage too high: {result.memory_peak}MB"
    ...
    def test_long_running_stability(self, perf_runner):
        ...
        assert result.success_rate > 0.95, f"Long-running stability issue: {result.success_rate:.2%}"
        assert result.latency_avg < 2.0, f"Performance degradation over time: {result.latency_avg}s"
    ...
    def test_stress_breaking_point(self, perf_runner):
        ...
        assert result.throughput >= 50, f"System breaking point too low: {result.throughput} users"
