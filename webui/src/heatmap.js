export function histogram(points, bins = 100, bounds = null) {
  ...
  if (bounds) {
    ...
    return [Array.from({ length: binsLat }, () => Array(binsLon).fill(0)), [0,0], [0,0]];
  }
  const hist = Array.from({ length: binsLat }, () => Array(binsLon).fill(0));
  if (maxLat === minLat || maxLon === minLon) return [hist, [minLat,maxLat],[minLon,maxLon]];
...
export function densityMap(points, bins = 100, bounds = null, radius = 1) {
  ...
  const dens = Array.from({ length: binsLon ? binsLat : 0 }, () => Array(binsLon).fill(0));
  for (let i=0;i<binsLat;i++) {
    for (let j=0;j<binsLon;j++) {
      ...
      for (let di=-radius; di<=radius; di++) {
        for (let dj=-radius; dj<=radius; dj++) {
          const ii = i+di;
          const jj = j+dj;
          if (ii>=0 && ii<binsLat && jj>=0 && jj<binsLon) dens[ii][jj] += count;
        }
      }
    }
  }
