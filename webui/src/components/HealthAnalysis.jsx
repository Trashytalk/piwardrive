...
export default function HealthAnalysis() {
    ...
    const load = () => {
      fetch('/status?limit=50')
        ...
        .then((recs) => {
          ...
          const temps = recs
            .map((r) => r.system.cpu_temp)
            .filter((x) => x != null);
          const avgTemp = temps.reduce((a, b) => a + b, 0) / temps.length;
          const mem = recs
            .map((r) => r.system.mem_percent)
            .filter((x) => x != null);
          const avgMem = mem.reduce((a, b) => a + b, 0) / mem.length;
          const disk = recs
            .map((r) => r.system.disk_percent)
            .filter((x) => x != null);
  ...
  return (
    <div>
      <div>
        Temp:{stats.avgTemp.toFixed(1)}°C Mem:{stats.avgMem.toFixed(0)}% Disk:
        {stats.avgDisk.toFixed(0)}%
      </div>
      <Line
        data={{
          labels,
