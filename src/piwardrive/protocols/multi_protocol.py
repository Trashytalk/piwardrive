...
class CellularScanner:
    ...
    def _process_cellular_device(self, device_data: Dict[str, Any]):
        ...
        if device_id not in self.devices:
            ...
            device.signal_strength = device_data.get('signal_strength', device.signal_strength)
            device.last_seen = datetime.now()
...
class SDRInterface:
    ...
    def configure(self, center_frequency: float, sample_rate: float, gain: float):
        ...
        self.gain = gain
        logger.info(f"SDR configured: {center_frequency/1e6:.1f} MHz, {sample_rate/1e6:.1f} MS/s, {gain} dB")
...
class MultiProtocolManager:
    ...
    def start_protocol_scan(self, protocol: RadioProtocol, **kwargs):
        ...
        elif protocol in [RadioProtocol.CELLULAR_GSM, RadioProtocol.CELLULAR_LTE, RadioProtocol.CELLULAR_5G]:
    ...
    def get_devices_by_protocol(self, protocol: RadioProtocol) -> List[RadioDevice]:
        ...
        return [device for device in self.all_devices.values() 
...
def test_multi_protocol_support():
    ...
    manager.start_protocol_scan(RadioProtocol.SDR_GENERIC, 
