...
class WiFiPositioning:
    ...
    def add_access_point(self, mac: str, position: Position, 
                        tx_power: float = 20.0):
    ...
    def add_fingerprint(self, position: Position, 
    ...
    def estimate_position_trilateration(self, 
                                      scan_results: List[Dict[str, Any]]) -> Optional[Position]:
        ...
        for result in scan_results:
            ...
            if mac in self.access_points:
                ...
                distance = self._rssi_to_distance(rssi, 
                    self.access_points[mac]['tx_power'])
    ...
    def estimate_position_fingerprinting(self, 
                                       scan_results: List[Dict[str, Any]]) -> Optional[Position]:
        ...
        current_fp = {result['mac']: result['rssi'] 
                     for result in scan_results}
    ...
    def _trilaterate(self, aps: List[Dict[str, Any]]) -> Optional[Position]:
        ...
        return Position(
            x=x, y=y, z=0.0, floor=avg_floor,
            accuracy=max(distances) * 0.1,
            source="trilateration"
        )
    
    def _compare_fingerprints(self, fp1: Dict[str, float], 
                             fp2_entry: Dict[str, Any]) -> float:
        """Compare two fingerprints using Euclidean distance"""
        fp2 = {result['mac']: result['rssi'] 
               for result in fp2_entry['scan_results']}
...
class CompassSystem:
    ...
    def get_heading_to_target(self, current_pos: Position, 
                             target_pos: Position) -> float:
...
class DeadReckoning:
    ...
    def update_position(self, heading: float, steps: int = 1, 
                       time_delta: float = 1.0) -> Optional[Position]:
    ...
    def get_position_confidence(self) -> float:
        ...
        time_since_fix = (datetime.now() - 
                         self.position_history[-1].timestamp).total_seconds()
...
class Pathfinder:
    ...
    def find_path(self, start: str, end: str, 
                 algorithm: PathfindingAlgorithm = PathfindingAlgorithm.A_STAR) -> Optional[List[str]]:
...
class OfflineNavigationSystem:
    ...
    def update_position(self, scan_results: List[Dict[str, Any]] = None,
                       heading: float = None, steps: int = 0) -> Optional[Position]:
        """Update current position"""
        new_position = None
        
        if self.navigation_mode == NavigationMode.WIFI_POSITIONING and scan_results:
            # Try trilateration first
            new_position = self.wifi_positioning.estimate_position_trilateration(scan_results)
            
            # Fall back to fingerprinting
            if not new_position:
                new_position = self.wifi_positioning.estimate_position_fingerprinting(scan_results)
        
        elif self.navigation_mode == NavigationMode.DEAD_RECKONING and heading is not None:
    ...
    def create_route(self, waypoint_ids: List[str], route_id: str = None) -> Optional[Route]:
        ...
        # Create route
        route = Route(
            id=route_id or f"route_{int(time.time())}",
            waypoints=waypoints
        )
...
def demo_offline_navigation():
    ...
    if position:
        print(f"   Estimated position: ({position.x:.1f}, {position.y:.1f}) floor {position.floor}")
    ...
    if nav_system.current_position:
        ...
        if dr_position:
            print(f"   Dead reckoning position: ({dr_position.x:.1f}, {dr_position.y:.1f})")
            print(f"   Confidence: {nav_system.dead_reckoning.get_position_confidence():.2f}")
    ...
    for i, bc in enumerate(nav_system.breadcrumbs[-3:]):  # Show last 3
        print(f"   Breadcrumb {i+1}: ({bc.position.x:.1f}, {bc.position.y:.1f}) "
