...
from piwardrive.security import sanitize_path, error_json, verify_password
...
async def control_service_endpoint(name: str, action: str, _auth: Any = service.AUTH_DEP) -> service.ServiceControlResponse:
    ...
    if not success:
        ...
        raise HTTPException(status_code=500, detail=error_json(500, msg or "command failed"))
...
async def update_config_endpoint(updates: dict[str, Any], _auth: Any = service.AUTH_DEP) -> service.ConfigResponse:
    ...
    for key, value in updates.items():
        if key not in data:
            raise HTTPException(status_code=400, detail=error_json(400, f"Unknown field: {key}"))
        data[key] = value
