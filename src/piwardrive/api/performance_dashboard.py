...
from piwardrive.performance.db_optimizer import DatabaseOptimizer, run_performance_analysis
...
async def get_database_stats(db_path: Optional[str] = None, _auth: Any = Depends(AUTH_DEP)):
    ...
    if not db_path:
        ...
        try:
            ...
            return JSONResponse({"error": "Database path not specified"}, status_code=400)
...
async def get_comprehensive_stats(db_path: Optional[str] = None, _auth: Any = Depends(AUTH_DEP)):
...
async def get_performance_history(
    ...
    while current_time < end_time:
        ...
        history["metrics"]["async_operations_per_second"].append({
            "timestamp": timestamp,
            "value": 100 + (hash(timestamp) % 200)
        })
        
        history["metrics"]["websocket_connections"].append({
            "timestamp": timestamp,
            "value": max(0, 10 + (hash(timestamp) % 50))
        })
...
async def get_performance_recommendations(db_path: Optional[str] = None, _auth: Any = Depends(AUTH_DEP)):
    ...
    if db_path:
        try:
            ...
            large_tables = [name for name, stats in table_stats.items() if stats["size_mb"] > 100]
            if large_tables:
                recommendations.append({
                    "category": "database",
                    "priority": "medium",
    ...
    try:
        ...
        if 'operation_summary' in async_summary:
            slow_operations = [
                op for op, stats in async_summary['operation_summary'].items()
            ...
            if slow_operations:
                recommendations.append({
                    ...
                    "action": "Review and optimize slow async operations: " + ", ".join(slow_operations[:3]),
