...
from piwardrive.security import error_json
...
@router.get("/status")
async def get_status(limit: int = 5, _auth: Any = service.AUTH_DEP) -> list[service.HealthRecordDict]:
...
async def baseline_analysis_endpoint(limit: int = 10, days: int = 30, threshold: float = 5.0, _auth: Any = service.AUTH_DEP) -> service.BaselineAnalysisResult:
...
@router.post("/sync")
async def sync_records(limit: int = 100, _auth: Any = service.AUTH_DEP) -> service.SyncResponse:
    records = db_service.load_recent_health(limit)
    if inspect.isawaitable(records):
        records = await records
