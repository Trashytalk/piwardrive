...
async def get_dashboard_settings_endpoint(_auth: Any = service.AUTH_DEP) -> service.DashboardSettingsResponse:
    settings = await db_service.load_dashboard_settings()
    return {"layout": settings.layout, "widgets": settings.widgets}

@router.post("/dashboard-settings")
async def update_dashboard_settings_endpoint(data: dict[str, Any], _auth: Any = service.AUTH_DEP) -> service.DashboardSettingsResponse:
    layout = data.get("layout", [])
    widgets = data.get("widgets", [])
    await db_service.save_dashboard_settings(service.DashboardSettings(layout=layout, widgets=widgets))
