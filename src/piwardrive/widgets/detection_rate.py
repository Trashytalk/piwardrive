"""Widget displaying live detection rate."""

import logging
from typing import Any

from piwardrive.simpleui import Card as MDCard
from piwardrive.simpleui import Label as MDLabel
from piwardrive.simpleui import dp

from .base import DashboardWidget


class DetectionRateWidget(DashboardWidget):
    """Show live detection rate from analytics stats."""

    update_interval = 5.0

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.card = MDCard(orientation="vertical", padding=dp(8), radius=[8])
        self.label = MDLabel(text="Detection Rate: N/A", halign="center")
        self.card.add_widget(self.label)
        self.add_widget(self.card)
        self.update()

    def update(self) -> None:
        try:
            # Placeholder until analytics integration is implemented
            self.label.text = "Detection Rate: N/A"
        except Exception as exc:  # pragma: no cover - UI update
            logging.exception("DetectionRateWidget update failed: %s", exc)
