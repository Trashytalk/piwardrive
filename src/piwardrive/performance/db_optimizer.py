...
class DatabaseOptimizer:
    ...
    async def analyze_query_performance(self, days: int = 7) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "slowest_queries": sorted(slow_queries, key=lambda x: x.execution_time, reverse=True)[:10]
        }
    ...
    async def get_table_stats(self) -> Dict[str, Any]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            for table_name, create_sql in tables:
                ...
                cursor = await db.execute(f"SELECT SUM(pgsize) FROM dbstat WHERE name='{table_name}'")
    ...
    async def vacuum_database(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "size_reduction_percent": ((size_before - size_after) / size_before * 100) if size_before > 0 else 0
        }
    ...
    def record_query_metrics(self, query: str, execution_time: float, row_count: int):
        ...
        # Log slow queries
        if metric.is_slow:
            logger.warning(f"Slow query detected: {execution_time:.3f}s, {row_count} rows: {query[:200]}...")
...
class OptimizedSQLiteConnection:
    ...
    async def execute_many(self, query: str, parameters_list: List[Tuple]) -> None:
        ...
        # Record metrics if optimizer is available
        if self.optimizer:
            self.optimizer.record_query_metrics(query, execution_time, len(parameters_list))
...
async def apply_performance_optimizations(db_path: str, create_indexes: bool = True) -> Dict[str, Any]:
    ...
    if create_indexes:
        ...
        if recommendations:
            results["index_creation"] = await optimizer.create_recommended_indexes(recommendations)
