...
class DFIntegrationManager:
    ...
    def _create_df_measurement(self, data: Dict[str, Any]) -> Optional[DFMeasurement]:
        ...
        try:
            ...
            if not all(field in data for field in required_fields):
                logger.warning(
                    f"Measurement missing required fields: {required_fields}"
                )
                return None
    ...
    async def _process_measurement_batch(self, measurements: List[DFMeasurement]):
        ...
        try:
            ...
            self.integration_metrics["failed_estimates"] += len(measurements) - len(
                results
            )
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "df_engine_status": (
                self.df_engine.get_algorithm_status() if self.df_engine else {}
            ),
            "hardware_status": (
                self.df_engine.get_hardware_status() if self.df_engine else {}
            ),
    ...
    async def calibrate_df_system(self, calibration_data: Dict[str, Any]) -> bool:
        ...
        try:
            if self.df_engine:
                ...
                if success:
                    ...
                    await self._notify_callbacks(
                        "calibration_completed", {"success": True}
                    )
                else:
                    logger.error("DF system calibration failed")
                    await self._notify_callbacks(
                        "calibration_completed", {"success": False}
                    )
            ...
            await self._notify_callbacks(
                "calibration_completed", {"success": False, "error": str(e)}
            )
            return False
