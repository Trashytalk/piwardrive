...
class WiFiAdapterConfig:
    ...
    supported_chipsets: List[str] = field(
...
class BluetoothClassicConfig:
    ...
    tracking_algorithm: str = "kalman"  # 'kalman', 'particle_filter', 'ek'
...
class IoTProtocolConfig:
    ...
    supported_protocols: List[str] = field(
        default_factory=lambda: ["zigbee", "z-wave", "thread"]
    )
    enable_mesh_analysis: bool = True
    enable_topology_mapping: bool = True
    topology_algorithms: List[str] = field(
        default_factory=lambda: ["dijkstra", "centrality"]
    )
...
class DFConfiguration:
    ...
    # Core DF settings
    enabled_algorithms: List[DFAlgorithm] = field(
        default_factory=lambda: [DFAlgorithm.RSS_TRIANGULATION]
    )
    ...
    array_processing: ArrayProcessingConfig = field(
        default_factory=ArrayProcessingConfig
    )
    visualization: VisualizationConfig = field(default_factory=VisualizationConfig)
    ble: BLEConfig = field(default_factory=BLEConfig)

    # Phase 3 configurations
    bluetooth_classic: BluetoothClassicConfig = field(
        default_factory=BluetoothClassicConfig
    )
...
class DFConfigManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        self.config_path = (
            config_path or Path.home() / ".config" / "piwardrive" / "df_config.json"
        )
        ...
        if algorithm in self._config.enabled_algorithms:
            ...
            if self._config.primary_algorithm == algorithm:
                if self._config.enabled_algorithms:
                    ...
                    logger.info(
                        f"Switched primary algorithm to: {self._config.primary_algorithm.value}"
                    )
                else:
    ...
    def get_algorithm_config(
        self, algorithm: Union[str, DFAlgorithm]
    ) -> Dict[str, Any]:
