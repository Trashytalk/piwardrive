...
from .config import WiFiAdapterConfig, AntennaArrayConfig, ArrayGeometry
...
class HardwareDetector:
    ...
    def check_injection_support(adapter_name: str) -> bool:
        ...
        try:
            ...
            injection_capable = ['ath9k', 'rt2800usb', 'mt7601u', 'rtl8812au', 'rtl8821au']
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        for adapter in self.adapters.values():
            if hasattr(adapter, 'performance_metrics'):
                ...
                total_metrics['packets_processed'] += metrics.get('packets_processed', 0)
...
class WiFiAdapter:
        ...
        try:
            ...
            if self.config.monitor_mode_required:
                if not self._check_monitor_mode_support():
                    raise RuntimeError(f"Adapter {self.name} does not support monitor mode")
    ...
    async def _set_power_level(self):
        ...
        try:
            await self._run_command(['iw', self.name, 'set', 'txpower', 'fixed', f'{self.config.power_level}00'])
            logger.debug(f"Set {self.name} power level to {self.config.power_level} dBm")
    ...
    async def _run_command(self, command: List[str]):
        ...
        try:
            ...
            if process.returncode != 0:
                raise RuntimeError(f"Command failed: {' '.join(command)}, Error: {stderr.decode()}")
...
class AntennaArrayManager:
    ...
    async def start(self):
        ...
        try:
            ...
            self.is_running = True
            self.is_initialized = True
            logger.info(f"Antenna array manager started with {self.config.num_elements} elements")
    ...
    def get_steering_vector(self, angle_deg: float) -> np.ndarray:
        ...
        return self.steering_vectors.get(closest_angle, np.ones(self.config.num_elements, dtype=complex))
    ...
    def apply_temperature_compensation(self, signal_data: np.ndarray, temperature: float) -> np.ndarray:
    ...
    async def calibrate(self, calibration_data: Dict[str, Any]):
        ...
        if 'calibration_signals' in calibration_data:
            ...
            logger.info(f"Calibrating antenna array with {len(signals)} calibration signals")
