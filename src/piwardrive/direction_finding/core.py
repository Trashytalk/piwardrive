...
from .config import DFConfiguration, DFAlgorithm, get_df_config
...
class DFEngine:
    ...
    def _initialize_hardware(self):
        ...
        from .hardware import WiFiAdapterManager, AntennaArrayManager
        
        try:
            self.hardware_managers['wifi'] = WiFiAdapterManager(self.config.wifi_adapter)
            self.hardware_managers['antenna'] = AntennaArrayManager(self.config.antenna_array)
    ...
    async def process_measurements(self, measurements: List[DFMeasurement]) -> List[DFResult]:
    ...
    async def _process_target(self, bssid: str, measurements: List[DFMeasurement]) -> Optional[DFResult]:
        ...
        if not primary_algorithm:
            logger.error(f"Primary algorithm {self.config.primary_algorithm.value} not available")
            return None
        ...
        try:
            ...
            if not result and self.config.fallback_algorithm:
                ...
                if fallback_algorithm:
                    result = await self._run_algorithm(fallback_algorithm, bssid, measurements)
    ...
    async def _run_algorithm(self, algorithm, bssid: str, measurements: List[DFMeasurement]) -> Optional[DFResult]:
        ...
        try:
            if hasattr(algorithm, 'process_async'):
                ...
                logger.error(f"Algorithm {algorithm.__class__.__name__} has no process method")
                return None
        except Exception as e:
            logger.error(f"Algorithm {algorithm.__class__.__name__} failed: {e}")
            return None
    
    def _update_performance_metrics(self, successful_results: int, processing_time: float):
        """Update performance metrics."""
    ...
    def get_algorithm_status(self) -> Dict[str, Any]:
        ...
        for algorithm_type, algorithm in self.algorithms.items():
            status[algorithm_type.value] = {
                'available': True,
                'initialized': hasattr(algorithm, 'is_initialized') and algorithm.is_initialized,
    ...
    def get_hardware_status(self) -> Dict[str, Any]:
        ...
        for manager_type, manager in self.hardware_managers.items():
            status[manager_type] = {
                'available': True,
                'initialized': hasattr(manager, 'is_initialized') and manager.is_initialized,
