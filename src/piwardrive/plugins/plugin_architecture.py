...
class PluginValidator:
    ...
    def __init__(self):
        self.allowed_imports = {
            ...
            'scipy', 'sklearn', 'json', 'csv', 'datetime', 'time',
            'os', 'sys', 'pathlib', 'logging', 'threading', 'queue'
        }
...
class PluginManager:
    ...
    def get_active_plugins(self) -> List[str]:
        ...
        return [
            plugin_id for plugin_id, info in self.plugins.items()
    ...
    def _create_data_access(self, plugin_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'write_file': lambda filename, data: self._safe_write_file(data_dir / filename, data),
...
class ExampleVisualizationPlugin(VisualizationPlugin):
    ...
    def render(self, data: Any, config: Dict[str, Any]) -> str:
        """Render visualization"""
        return f"<div>Example visualization for {len(data) if hasattr(data, '__len__') else 'data'} items</div>"
...
def demo_plugin_architecture():
    ...
    analysis_result = plugin_api.call_endpoint('execute_plugin', 'analysis_example', test_data)
